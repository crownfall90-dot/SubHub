    ...
    async def _poll():
        ...
        async def _bg_bitrefill_import(cid):
            ...
            try:
                ...
                return
            await _send(cid, "🎁 <b>Импорт из Bitrefill</b>\n"
                             "━━━━━━━━━━━━━━━━━━━\n"
                             f"{escape_html(report)}", parse_mode="HTML")
        ...
        async def _handle_cbq(client, cbq):
            ...
            if data.startswith("gift:add_denom:"):
                ...
                await _send(cid,
                    ...
                    "*Текстом* — каждая карта с новой строки, `серия` и `PIN` через "
                    "пробел (дата не нужна):\n"
                    "`6000170524661453 281697`\n\n"
                    "*Файлом* — как экспорт Flipkart (Серия, PIN, дата — дата "
                    "игнорируется).",
                    parse_mode="Markdown",
