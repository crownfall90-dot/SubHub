...
class GrizzlySMSClient:
    ...
    async def get_number_parallel(
        ...
        Без providerIds — любой оператор. maxPrice в API + локальный отказ
        если activationCost выше потолка.
        ...
        async def _slot(slot_idx: int) -> Tuple[str, str, float]:
            ...
            while True:
                ...
                try:
                    ...
                    # Жёсткий потолок: дороже max_price — отмена и продолжаем искать
                    # (любой оператор ок, лишь бы ≤ cap).
                    if max_price is not None and cost > float(max_price) + 1e-9:
                        logger.warning(
    ...
    async def get_number(
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                if max_price is not None and cost > float(max_price) + 1e-9:
                    logger.warning(
                        f"Grizzly ${cost:.4f} > max ${float(max_price):.2f} — отмена {activation_id}"
                    )
                    ...
                    if attempt < retries:
                        ...
                        continue
                    raise NumberUnavailableError(
                        f"Только номера дороже max_price ${float(max_price):.2f}"
                    )
