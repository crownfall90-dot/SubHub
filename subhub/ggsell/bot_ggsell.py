...
class GGSellBotHandler:
    ...
    def _order_months(self, order: dict) -> int:
        """Реальный срок подписки из ВЫБРАННОЙ опции заказа (1, 3, 6, 12 — что
        покупатель фактически выбрал). Название товара не смотрим — там всегда
        все варианты. Используется для отображения покупателю/продавцу и для
        триггера напоминания о повторной активации (>=6 мес); фактически
        покупаемый на Flipkart тариф всегда 3 мес — см. `buy_months` в
        bg_fulfill_order/handle_auto_fulfill, там раздельная, не бакетированная
        логика. По умолчанию (нет опции с числом) — 3."""
        try:
            p = self.parse_order(order)
    ...
    async def bg_fulfill_order(self, invoice_id: int, order: dict, cid=None, mid=None) -> None:
        ...
        # Flipkart-автоматизация всегда покупает тариф 3 мес в этом флоу (как и в
        # ручном auto-fulfill ниже, см. `months = 3` в handle_auto_fulfill) — GGSell
        # продаёт 3/6("12 переименован в 6")/1("без входа") варианты, но реального
        # отдельного тарифа на 1 или 6/12 мес на Flipkart нет: 6 мес = 2× 3 мес
        # (3 сейчас + 3 через 3 мес вручную), остальное сводится к тем же 3 мес.
        buy_months = 3
