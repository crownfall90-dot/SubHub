...
from common import (                      # noqa: E402  (общие помощники)
    ...
    dir_size as _dir_size,
    fmt_msk as _fmt_msk,
)
from console_ui import (                  # noqa: E402  (цвета и примитивы вывода)
    R, G, Y, C, M, B, W, DIM, BLD, RST,
    cls, pause, header, section, opt,
)
...
def request_host_restart(source: str = "telegram") -> str:
    ...
    _shutting_down = True
    _console_ui.set_shutting_down(True)   # pause() живёт в console_ui
...
async def _open_chrome_keep_alive(profile_path: Path, url: str | None = None) -> None:
    ...
    try:
        ...
        _how = "через прокси" if proxy else "напрямую"
        print(f"  {DIM}Открываю Flipkart (+91 {tag}) {_how} → {target[:80]}…{RST}")
...
# Проверка наличия — один HTTP-запрос (~0.3с), браузер поднимается только как
# запасной путь. Товар разбирают быстро, поэтому спрашиваем часто; при сбоях
# интервал растёт до получаса, чтобы не долбить сайт впустую.
_BITREFILL_CHECK_EVERY = 2 * 60
